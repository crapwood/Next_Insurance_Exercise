import {
  ...
  listen,
  query,
  handleTitleGreaterThanLessThan,
} from "./helpers.js";
...
export default class View {
  ...
  bindAddItem(handler) {
    listen(this.$newTodo, "change", ({ target }) => {
      ...
      let flag = false;
      let data = JSON.parse(sessionStorage.getItem("todos-vanilla-es6"));
      data.map((item) => {
        if (item["title"] === title) {
          flag = true;
        }
      });
      if (flag) alert("Task already exist");
      else {
        var temp = title;
        if (title) {
          let newTitle = handleTitleGreaterThanLessThan(temp);
          handler(newTitle);
        }
      }
    });
  ...
  bindToggleItem(handler) {
    Event(this.$todoList, ".toggle", "click", ({ target }) => {
